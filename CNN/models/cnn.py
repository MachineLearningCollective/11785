...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        last_conv_output_size = input_width  # Initialize with input width
        for i in range(self.nlayers):
            conv_layer = Conv1d(num_input_channels if i == 0 else num_channels[i - 1],
                                num_channels[i],
                                kernel_sizes[i],
                                strides[i],
                                0,
            ...
            # Update the last_conv_output_size within the loop
            last_conv_output_size = (last_conv_output_size - kernel_sizes[i]) // strides[i] + 1

        # Flatten the output from the last convolutional layer
        self.linear_input_size = last_conv_output_size * num_channels[-1]
        self.flatten = Flatten()

        # Initialize the linear layer with the correct input size
        self.linear_layer = Linear(self.linear_input_size, num_linear_neurons)
        self.linear_layer.W = linear_weight_init_fn(num_linear_neurons, self.linear_input_size)
    ...
    def backward(self, labels):
        ...
        # Backpropagate through convolutional layers in reverse order
