...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        # vectors given the paramters. [(seq_len+1) * (num_layers, batch_size, hidden_size)]
        ...
        logits = np.zeros((batch_size, self.hidden_size), dtype=float)
        ...
        for t in range(seq_len):
            for layer_idx, rnn in enumerate(self.rnn):
                # For the first layer, use the input x, otherwise, use the output of the previous layer
                if layer_idx == 0:
                    h_current_t = rnn.forward(x[:, t, :], hidden[layer_idx])
                else:
                    h_current_t = rnn.forward(hidden[layer_idx - 1], hidden[layer_idx])
                hidden[layer_idx] = h_current_t
            self.hiddens.append(hidden.copy())

        # Calculate logits using the output layer from the last hidden state
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in reversed(range(seq_len)):
            hidden_at_t = self.hiddens[t+1]
            for layer_idx in reversed(range(self.num_layers)):
                rnn = self.rnn[layer_idx]

                # get current hidden state
                h_current = hidden_at_t[layer_idx]

                # get previous layer current hidden state
                h_prev_l = self.x[:, t, :] if layer_idx == 0 else hidden_at_t[layer_idx - 1]

                # get previous time current state
                h_prev_t = self.hiddens[t][layer_idx]

                dx, dh_prev_t = rnn.backward(dh[layer_idx], h_current, h_prev_l, h_prev_t)
