...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        if eval == False:
            ...
            # Update running mean and variance using batch statistics
            self.running_M = self.M * self.alpha + self.running_M * (1 - self.alpha)
            self.running_V = self.V * self.alpha + self.running_V * (1 - self.alpha)
        else:
            # inference mode
    ...
    def backward(self, dLdBZ):
        ...
        dzdM = - (self.V + self.eps) ** (- 0.5) - 0.5 * (self.Z - self.M) * (self.V + self.eps) ** (- 1.5) * (- 2 / self.N * np.sum(self.Z - self.M, axis = 0))
        dLdM = np.sum(dLdNZ * dzdM, axis = 0)
